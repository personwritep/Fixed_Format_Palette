...
// @grant        none
// @updateURL        https://github.com/personwritep/Fixed_Format_Palette/raw/main/Fixed_Format_Palette.user.js
// @downloadURL        https://github.com/personwritep/Fixed_Format_Palette/raw/main/Fixed_Format_Palette.user.js
// ==/UserScript==
...
function main(){
    ...
    function record_format_h(){
        document.execCommand("copy"); // HTMLコピーは「clipboard API」が使えない🔴

        let through=0;
        navigator.clipboard.readText().then( function(clipText){

            if(!check_html(clipText)){
                alert(
                    " ⛔ HTMLコードの選択範囲が適切ではありません\n\n"+
                    "　　選択範囲の開始タグ～終了タグが閉じている必要があります\n"+
                    "　　　➔➔➔　「OK」を押すと登録を中止します");
                through=1;
                select_format='';
                mode=-1;
        ...
        function check_html(code){
            for (let k=0; k<2000; k++){ // チェック回数の上限 🔴
                let result=check_tag(code);
                if(result==1){
                    return false; }
                else if(result==2){
                    return true; }
            ...
            function check_tag(str){
                let r_CS=str.indexOf('</');
                let r_CE=str.indexOf('>', r_CS+2);
                let Ctag=str.substring(r_CS, r_CE+1);
                ...
                let Ctagname=str.substring(r_CS+2, r_CE);
                let r_OS=str.lastIndexOf('<', r_CS-1);
                let r_OE=str.indexOf('>', r_OS+2);
                let Otag=str.substring(r_OS, r_OE+1);

                if(r_OS==-1){
                    return 1; }
                else{
                    let name=get_name(Otag);
                    if(name=='br' || name=='img' || name=='hr'){
                        let before=str.substring(0, r_OS);
                        let after=str.substring(r_OE+1);
                        code=before+after;
                        return 0; }
                    else if(name==Ctagname){
                        let before=str.substring(0, r_OS);
                        let after=str.substring(r_CE+1);
                        code=before+after;
                        return 0; }
                    else{
                        return 1; }}
                ...
                function get_name(tag){
                    let tag_name;
                    let r_C=tag.indexOf(' ', 2);
                    if(r_C==-1){
                        tag_name=tag.substring(1, tag.length-1); }
                    else{
                        tag_name=tag.substring(1, r_C); }
                    return tag_name; }
